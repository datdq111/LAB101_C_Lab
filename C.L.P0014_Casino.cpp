...
void buy_chip (int &money, int &chips);
void sell_chip(int &money, int &chips);
void game_craps(int &money, int &chips);
void game_Arup(int &money, int &chips);
//random dice
int dice_random(){
    int totalDice,
            dice1,
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        printf("6) Quit\n");
        int flag = scanf("%d%c", &urchoice, &enter); //int flag = scanf("%d,%c", &urchoice, &enter); 
        if (flag == 0 || enter != '\n') {
            while (getchar() != '\n');
...
void buy_chip (int &money, int &chips) {
    ...
    while (1) {
        ...
        if (flag == 0 || enter != '\n') {
            ...
            money += - money_buy_chip + money_buy_chip % 11; //	
            break;
        }
    }    
}
...
void program_casino(int urchoice, int &money, int &chips) {
    ...
    switch (urchoice) {
        ...
        case 1:
        {
            buy_chip(money, chips);
            break;
        }
            //Sell chips
        case 2:
        {
            sell_chip(money, chips);
            break;
        }
            //Game Craps
        case 3:
        {
            game_craps(money, chips);
            break;
        }
            //Game Arup's Game of Dice
        case 4:
        {
            game_Arup(money,chips);
            break;
        }
            //Status Report
        case 5:
